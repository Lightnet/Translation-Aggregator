...
extern const wchar_t userAgent[] = L"Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko";

int numHttpWindows = 0;
HINTERNET hHttpSession[2] = {0, 0};
...
wchar_t *HttpEscapeParamW(const wchar_t *src, int len)
{
	...
	while (len)
	{
		...
		if (c <= 0x26 || (0x3A <= c && c <= 0x40) || (0x5B <= c && c <= 0x60) || (0x7B <= c && c <= 0x7E))
		{
...
char *HttpEscapeParamA(const char *src, int len)
{
	...
	while (len)
	{
		len--;
		char c = *src;
		if (c <= 0x26 || (0x3A <= c && c <= 0x40) || c == '\\' || (0x5B <= c && c <= 0x60) || (0x7B <= c && c <= 0x7E))
		{
...
void HttpWindow::TryMakeRequest()
{
	...
	if (!postPrefixTemplate)
	{
		...
		ClearQueuedTask();
		wchar_t *path = GetTranslationPath(config.langSrc, config.langDst, string);
		free(string);
		hRequest = WinHttpOpenRequest(hConnect, L"GET", path, 0, referrer, WINHTTP_DEFAULT_ACCEPT_TYPES, port == 443 ? WINHTTP_FLAG_SECURE : 0);
		free(path);
	}
	else
	{
	...
	wchar_t *path = GetTranslationPath(config.langSrc, config.langDst, L"");
		hRequest = WinHttpOpenRequest(hConnect, L"POST", path, 0, referrer, WINHTTP_DEFAULT_ACCEPT_TYPES, port == 443 ? WINHTTP_FLAG_SECURE : 0);
	free(path);
	}
